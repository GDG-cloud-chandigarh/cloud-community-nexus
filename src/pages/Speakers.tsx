  ...
  return (
    <section className="container py-16">
      <Helmet>
        ...
        <meta name="description" content="Meet the Cloud Community Days speakers and community leaders." />
        <link rel="canonical" href="/speakers" />
      </Helmet>
      <h1 className="font-display text-4xl mb-8">Speakers</h1>
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {speakers.map((sp) => (
          <article key={sp.name} className="p-6 rounded-xl border bg-card shadow-soft text-center">
            <Avatar className="mx-auto h-16 w-16">
              <AvatarFallback>{sp.name.split(' ').map(s=>s[0]).join('')}</AvatarFallback>
            </Avatar>
            <h3 className="mt-4 font-medium">{sp.name}</h3>
            <p className="text-sm text-muted-foreground">{sp.title}</p>
            <div className="mt-3 text-xs text-muted-foreground">LinkedIn • Twitter</div>
          </article>
        ))}
      </div>
    </section>
